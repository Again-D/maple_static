...
describe("character image proxy", () => {
  it("rejects malformed, unsupported, and cache-busting requests before fetching", async () => {
    ...
    const unsupportedPath = await GET(new NextRequest("http://localhost/api/character-image?url=https%3A%2F%2Fopen.api.nexon.com%2Fstatic%2Fmaplestory%2Fother%2Fimage.png&v=1"));
